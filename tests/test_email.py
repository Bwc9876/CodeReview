...
class TestEmail(BaseCase):
    ...
    def test_created(self) -> None:
        self.post('student-affiliated', reverse("review-create"),
                  {"schoology_id": "12.34.56", "rubric": self.rubric.id})
        self.assertEqual(len(mail.outbox), 2)
        for msg in mail.outbox:
            self.assertTrue(
                msg.to[0] == self.users['reviewer-affiliated'].email or msg.to[0] == self.users['reviewer-not'].email,
    ...
    def test_session_checks(self) -> None:
        ...
        self.post('student-affiliated', reverse("review-create"), {"schoology_id": "12.34.56",
                                                                   "rubric": self.rubric.id})
        self.assertEqual(len(mail.outbox), 0)

    def test_not_self(self) -> None:
        self.set_user_session('reviewer-not', User.Session.PM)
        self.post('reviewer-not', reverse("review-create"), {"schoology_id": "12.34.56", "rubric": self.rubric.id})
        self.assertEqual(len(mail.outbox), 0)
