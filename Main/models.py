...
class Review(BaseModel):
    ...
    reviewer = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="reviewer", null=True, blank=True
    )
    schoology_id = models.CharField(
        max_length=10,
        null=True,
    ...
    additional_comments = models.TextField(
        blank=True, null=True, help_text="Any additional comments you have on the code"
    )
    ...
    def score_fraction(self) -> Optional[str]:
        ...
        if self.status == Review.Status.CLOSED:
            ...
            max_score = self.scoredrow_set.all().aggregate(
                models.Sum("source_row__max_score")
            )
