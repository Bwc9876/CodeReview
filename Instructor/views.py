    ...
    def post(self, request) -> HttpResponse:
        ...
        if password is None or password == "":
            messages.add_message(
                self.request, messages.ERROR, "Please provide a password"
            )
            return redirect("user-list")
        else:
            try:
                backend = (
                    LDAPMockAuthentication
                    if settings.AUTHENTICATION_BACKENDS[0]
                    == "Users.ldap_mock.LDAPMockAuthentication"
                    else LDAPAuthentication
                )
                backend().delete_old_users(self.request.user.username, password)
                messages.add_message(
                    self.request, messages.SUCCESS, "Users Cleared Successfully"
                )
                return redirect("user-list")
...
class AdminHomeView(LoginRequiredMixin, IsSuperUserMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs) -> dict[str, object]:
        ...
        active = main_models.Review.objects.exclude(
            status=main_models.Review.Status.CLOSED
        )
        completed = main_models.Review.objects.filter(
            status=main_models.Review.Status.CLOSED
        )
    ...
    def post(self, *args, **kwargs) -> HttpResponse:
        ...
        try:
            ...
            self.get_queryset().filter(
                id__in=self.request.POST.getlist("to_delete")
            ).delete()
...
class RubricCreateView(
    LoginRequiredMixin, IsSuperUserMixin, FormNameMixin, FormAlertMixin, CreateView
):
    """
...
class RubricEditView(
    LoginRequiredMixin, IsSuperUserMixin, FormNameMixin, FormAlertMixin, UpdateView
):
    """
    ...
    def form_valid(self, form) -> HttpResponse:
        ...
        for review in main_models.Review.objects.filter(
            rubric=edited_object, status=main_models.Review.Status.CLOSED
        ):
...
class RubricDupeView(LoginRequiredMixin, IsSuperUserMixin, View):
    ...
    def post(self, request, *args, **kwargs) -> HttpResponse:
        ...
        try:
            target_rubric: models.Rubric = models.Rubric.objects.select_related().get(
                id=kwargs.get("pk")
            )
            self.request = request
            ...
            new_rubric = models.Rubric.objects.create(
                name=new_name, max_score=target_rubric.max_score
            )
            new_rubric.save()
            for row in target_rubric.rubricrow_set.all():
                new_row = models.RubricRow.objects.create(
                    name=row.name,
                    description=row.description,
                    max_score=row.max_score,
                    parent_rubric_id=new_rubric.id,
                    index=row.index,
                )
                new_row.save()
                for cell in row.rubriccell_set.all():
                    new_cell = models.RubricCell.objects.create(
                        score=cell.score,
