...
class RubricForm(ModelForm):
    ...
    rubric = CharField(
        ...
        help_text="This rubric will be used by reviewers to grade code",
    )
    _validation_schema = {
        "type": "array",
        "minItems": 1,
    ...
    def __init__(self, *args, **kwargs):
        ...
        ] = models.RubricRow.max_length("name")
        self._validation_schema[
            "items", "properties", "description", "maxLength"
        ] = models.RubricRow.max_length("description")
        self._validation_schema[
            "items", "properties", "cells", "items", "properties", "description"
        ] = models.RubricCell.max_length("description")
    ...
    def save(self, commit=True) -> models.Rubric:
        ...
        for index, row in enumerate(new_obj):
            new_row, created = models.RubricRow.objects.get_or_create(
            ...
            for cell_index, cell in enumerate(row.get("cells", [])):
                ...
                new_cell, cell_created = models.RubricCell.objects.get_or_create(
                    parent_row=new_row,
                    index=cell_index,
                    defaults={
            ...
            new_row.rubriccell_set.filter(
                index__gt=len(row.get("cells", [])) - 1
            ).delete()
    ...
    def explain_json_error(error: ValidationError) -> str:
        ...
        if len(error.relative_path) > 0:
            ...
            offending_row_elem = (
                error.relative_path[1] if len(error.relative_path) > 1 else None
            )
            if offending_row_elem == "description" or offending_row_elem == "name":
                ...
                if "[] is too short" == error.message:
                    ...
                    offending_cell_elem = (
                        error.relative_path[3]
                        if len(error.relative_path) >= 4
                        else None
                    )
                    if (
                        offending_cell_elem == "score"
                        and "not of type 'number'" in error.message
                    ):
                        return f"Please enter a number for the score {location}"
                    elif offending_cell_elem == "score" and (
                        "max" in error.message or "minimum" in error.message
                    ):
    ...
    def clean(self) -> dict[str, object]:
        ...
        if raw_json is not None:
            try:
                errors = sorted(
                    self._json_validator.iter_errors(JSONDecoder().decode(raw_json)),
                    key=str,
                )
                [
                    self.add_error("rubric", self.explain_json_error(error) + ".")
                    for error in errors
                ]
