    ...
    *NEVER* **EVER** USE THIS IN PRODUCTION
    ...
    def setup_server(cls) -> None:
        ...
        if cls.server is None:
            cls.server = Server.from_definition(
                "fake_server",
    ...
    def get_connection(cls, username: str, password: str) -> Connection:
        ...
        dn = (
            cls.construct_dn(username)
            if username
            not in [k.split(",")[0].split("=")[1] for k in cls.raw_users.keys()]
            else f"cn={username},ou=EvilPeople,dc=example,dc=com"
        )
        conn = Connection(cls.server, dn, password, client_strategy=MOCK_SYNC)
